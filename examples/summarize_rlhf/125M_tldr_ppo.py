...
config = TRLConfig(
    train=TrainConfig(
        ...
        checkpoint_dir="ppo_pythia125m_sft_summarize_tldr",
        group_name="ppo_pythia125m_sft_summarize_tldr",
    ),
    model=ModelConfig(
        model_path="pvduy/pythia-125M-sft-summarize-tldr",
        num_layers_unfrozen=2,
    ),
    tokenizer=TokenizerConfig(
        tokenizer_path="pvduy/pythia-125M-sft-summarize-tldr",
...
def create_reward_fn():
    if os.environ.get("RANK", "0") == "0":
        ...
        rw_model.half()
        rw_model.eval()
        rw_device = 0  # set reward model device
        rw_model.to(rw_device)
        lsh = pickle.load(open("125M_data/lsh_table_hashing.pickle", "rb"))
        post_summary_dict = pickle.load(open("125M_data/post_summary_dict.pickle", "rb"))
...
if __name__ == "__main__":
    ...
    if 0:
        ...
        if not os.path.exists("125M_data"):
            os.makedirs("125M_data")
        pickle.dump(lsh, open("125M_data/lsh_table_hashing.pickle", "wb"))
        pickle.dump(post_summary_dict, open("125M_data/post_summary_dict.pickle", "wb"))
        pickle.dump(train_prompts, open("125M_data/train_prompts.pickle", "wb"))
        pickle.dump(val_prompts, open("125M_data/val_prompts.pickle", "wb"))
        reward_fn = create_reward_fn()
    else:
        train_prompts = pickle.load(open("125M_data/train_prompts.pickle", "rb"))
        val_prompts = pickle.load(open("125M_data/val_prompts.pickle", "rb"))
        reward_fn = create_reward_fn()
